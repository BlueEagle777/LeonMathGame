...
public class MinusEquations : MonoBehaviour
{
    private StarManager starManager; // Add a reference to the StarManager
    // Singleton instance
    private static MinusEquations instance;

    public static MinusEquations Instance
    {
        get
        {
            if (instance == null)
            {
                instance = FindObjectOfType<MinusEquations>();
                if (instance == null)
                {
                    GameObject managerObject = new GameObject("MinusEquations");
                    instance = managerObject.AddComponent<MinusEquations>();
                }
            }
            return instance;
        }
    ...
    public struct SubtractionEquation
    {
        ...
        public SubtractionEquation(int firstNumber, int secondNumber, int playerAnswer, bool isCorrect, int time)
        {
    ...
    public List<SubtractionEquation> equationList = new List<SubtractionEquation>();
    ...
    public List<SubtractionEquation> GetEquations()
    {
        return equationList;
    }
    ...
    {
        SubtractionEquation equation = new SubtractionEquation(firstNumber, secondNumber, playerAnswer, isCorrect, time);
    ...
    private IEnumerator SaveEquationsToDatabase()
    {
        if (equationList.Count > 1)
        {
            ...
            string url = starManager.GetURL("save_equations_min.php"); // Replace with your PHP script URL
