...
for ($i = 0; $i < count($firstNumbers); $i++) {
    ...
    $sql = "INSERT INTO mul_hist (player_ID, Date, FirstNum, SecondNum, PlayerAns, Correct, Time) VALUES (?, ?, ?, ?, ?, ?, ?)";
...
$parameter = "mil"; // Set the parameter to "plus," "min," or "mul" as needed

// Check if the parameter is "mul"
if ($parameter === "mul") {
    // Query the "mul_hist" table to count correct answers
    $sql = "SELECT SUM(Correct) AS correctCount, COUNT(*) AS totalCount FROM mul_hist WHERE player_ID = ?";
    $stmt = $conn->prepare($sql);
    ...
    if ($result->num_rows > 0) {
        ...
        // Update the accuracy_mul value in the players table for the current player
        $updateSql = "UPDATE players SET accuracy_mul = ? WHERE player_ID = ?";
